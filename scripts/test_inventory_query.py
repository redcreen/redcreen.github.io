...
class JushuitanAPI:
    ...
    def _generate_sign(self, params):
        ...
        # 拼接字符串：app_secret + key1value1 + key2value2 + ... + app_secret
    ...
    def _build_params(self, biz_content=None):
        """构建请求参数"""
        timestamp = int(time.time())  # 使用UNIX时间戳（秒）
        params = {
        ...
        if biz_content:
            params['biz_content'] = json.dumps(biz_content, separators=(',', ':'), ensure_ascii=False)
    ...
    def call_api(self, api_path, biz_content=None):
        ...
        try:
            ...
            print(f"[响应] 内容: {json.dumps(result, ensure_ascii=False, indent=2)}")
            return result
...
if __name__ == '__main__':
    ...
    # 询问是否有access_token
    access_token = input("请输入access_token (如果没有请直接回车): ").strip()
    
    # 创建客户端
    client = JushuitanAPI(APP_KEY, APP_SECRET, access_token if access_token else None)
    ...
    if result.get('code') == 0:
        ...
        if result.get('code') == 140:
            print("\n提示: 错误码140表示验证失败")
            print("   可能原因：")
            print("   1. 签名算法不正确")
            print("   2. 缺少access_token")
            print("   3. 应用未授权")
            print("\n   建议：")
            print("   - 请确认是否已获取access_token")
            print("   - 检查聚水潭开放平台应用授权状态")
