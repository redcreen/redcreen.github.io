...
with open('inventory_result.txt', 'w', encoding='utf-8') as f:
    f.write(f"SKU: {inv['sku_id']}\n")
    f.write(f"名称: {inv['name']}\n")
    f.write(f"库存: {inv['qty']}\n")
    f.write(f"订单占用: {inv['order_lock']}\n")
    f.write(f"仓库锁定: {inv['pick_lock']}\n")
    f.write(f"采购在途: {inv['purchase_qty']}\n")

print("结果已写入 inventory_result.txt")
